...
//    }
    ...
    {
        return __('filament-authentication-log.navigation.group');
    }

    public static function getLabel(): string
    {
        return __('filament-authentication-log.navigation.authentication-log.label');
    }

    public static function getPluralLabel(): string
    {
        return __('filament-authentication-log.navigation.authentication-log.plural-label');
    }
...
//                    ->label(trans('filament-authentication-log.column.authenticatable'))
//                    ->formatStateUsing(function (?string $state, Model $record) {
//                        if (! $record->authenticatable_id) {
//                            return new HtmlString('&mdash;');
//                        }
//
//                        return $record->authenticatable_id;
//
...
//                    ->sortable(),
                ...
                Tables\Columns\TextColumn::make('ip_address')
                    ->label(trans('filament-authentication-log.column.ip_address'))
                    ->searchable()
                    ->sortable(),
                Tables\Columns\TextColumn::make('user_agent')
                    ->label(trans('filament-authentication-log.column.user_agent'))
                    ->searchable()
                    ->sortable()
                    ->limit(20)
                ...
                Tables\Columns\TextColumn::make('login_at')
                    ->label(trans('filament-authentication-log.column.login_at'))
                    ->dateTime()
                    ->sortable(),
                Tables\Columns\IconColumn::make('login_successful')->label('Login')
                    //->label(trans('filament-authentication-log.column.login_successful'))
                    ->boolean()
                    ->sortable(),
                Tables\Columns\TextColumn::make('logout_at')
                    ->label('Logout')
                    //->label(trans('filament-authentication-log.column.logout_at'))
                    ->dateTime()
                    ->sortable(),
                Tables\Columns\IconColumn::make('cleared_by_user')
                    ->label(trans('filament-authentication-log.column.cleared_by_user'))
                    ->boolean()
                    ->sortable(),
